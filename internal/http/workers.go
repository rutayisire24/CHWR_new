package http

import (
	"encoding/base64"
	"errors"
	"net/http"
	"strconv"
	"strings"

	"chwr/internal/auth"
	"chwr/internal/domain"
	"chwr/internal/store"
)

type workersPage struct {
	Workers  []domain.HealthWorker
	Matching int64
	Filter   filterView
	// PrevURL and NextURL are empty when there is no page that way, so the
	// template asks a string rather than reassembling the query itself.
	PrevURL   string
	NextURL   string
	Cadres    []cadreOption
	Places    []domain.Place // the chosen location's chain, for the "filtered to" line
	Districts []districtOption
	Prefill   map[string]int64
	CanEdit   bool
	// ExportURL carries the listing's own query string, so the file holds what
	// the page is showing rather than the whole register.
	ExportURL string
	CanExport bool
}

// filterView is the filter as the form redisplays it.
type filterView struct {
	Query      string
	Cadre      string
	Status     string
	LocationID int64
	Active     bool // any filter set at all
}

// cadreOption is one cadre as a filter choice (by slug) or a form radio (by
// id). Level drives the placement cascade: it is how deep the selects go.
type cadreOption struct {
	ID       int16
	Slug     string
	Label    string
	Level    string // the placement level this cadre requires
	Selected bool
}

type sexOption struct {
	Value    domain.Sex
	Label    string
	Selected bool
}

type workerFormPage struct {
	Action    string
	Worker    domain.HealthWorker
	Age       string // kept as typed, so a rejected form redisplays it
	Cadres    []cadreOption
	Sexes     []sexOption
	Districts []districtOption
	// Prefill is the location chain of an existing placement, so the cascade
	// can be rebuilt on edit without the browser guessing.
	Prefill    map[string]int64
	Duplicates []domain.HealthWorker
	Errors     map[string]string
}

type workerShowPage struct {
	Worker    domain.HealthWorker
	Placement []domain.Place
	Profile   domain.Profile
	Tools     []domain.CHWTool
	Domains   []domain.CHWServiceDomain
	// History is every posting the worker has held, newest first.
	History []domain.Deployment
	// Facilities is the supervising-facility picker for the open posting,
	// assembled only when there is one and the reader may change it.
	Facilities []facilityOption
	CanEdit    bool
}

func (s *Server) workersList(w http.ResponseWriter, r *http.Request) {
	sc := auth.ScopeFrom(r.Context())

	cadres, err := s.store.Deployments.Cadres(r.Context())
	if err != nil {
		s.fail(w, r, err)
		return
	}
	f, view := decodeFilter(r, cadres)

	page, err := s.store.Workers.List(r.Context(), sc, f)
	if err != nil {
		s.fail(w, r, err)
		return
	}
	matching, err := s.store.Workers.Matching(r.Context(), sc, f)
	if err != nil {
		s.fail(w, r, err)
		return
	}
	districts, err := s.store.Locations.Districts(r.Context(), sc)
	if err != nil {
		s.fail(w, r, err)
		return
	}

	// A location filter redisplays as the chain it names, so the page can say
	// which parish, in which subcounty, in which district.
	var places []domain.Place
	prefill := map[string]int64{}
	if f.LocationID != 0 {
		places, err = s.store.Locations.Ancestors(r.Context(), sc, f.LocationID)
		if err != nil && !errors.Is(err, domain.ErrNotFound) {
			s.fail(w, r, err)
			return
		}
		for _, place := range places {
			prefill[string(place.Level)] = place.ID
		}
	}

	options := make([]districtOption, 0, len(districts))
	for _, d := range districts {
		options = append(options, districtOption{
			ID: d.ID, Name: d.Name,
			Selected: prefill[string(domain.LevelDistrict)] == d.ID,
		})
	}

	s.render(w, r, http.StatusOK, "workers", workersPage{
		Workers:   page.Workers,
		Matching:  matching,
		Filter:    view,
		PrevURL:   pageURL(r, page.HasPrev, "before", page.First),
		NextURL:   pageURL(r, page.HasNext, "after", page.Last),
		Cadres:    cadreOptions(cadres, view.Cadre),
		Places:    places,
		Districts: options,
		Prefill:   prefill,
		CanEdit:   auth.Can(auth.MustUser(r.Context()).Role, auth.CapWorkerCreate),
		ExportURL: exportURL(r),
		CanExport: auth.Can(auth.MustUser(r.Context()).Role, auth.CapExport),
	})
}

// cadreOptions lays the cadre vocabulary out for a filter or a form, marking
// the slug that is selected, if any.
func cadreOptions(cadres []domain.Cadre, selectedSlug string) []cadreOption {
	out := make([]cadreOption, 0, len(cadres))
	for _, c := range cadres {
		out = append(out, cadreOption{
			ID: c.ID, Slug: c.Slug, Label: c.Label,
			Level:    string(c.PlacementLevel),
			Selected: c.Slug == selectedSlug,
		})
	}
	return out
}

// decodeFilter reads the filter from the query string. Everything is optional,
// and anything unrecognised is dropped rather than rejected: a filter arrives
// from a bookmarked or shared URL as often as from the form, and a stale one
// should show the register rather than an error.
func decodeFilter(r *http.Request, cadres []domain.Cadre) (store.Filter, filterView) {
	q := r.URL.Query()

	f := store.Filter{Query: strings.TrimSpace(q.Get("q"))}
	view := filterView{Query: f.Query}

	// The cadre filter is a slug from the vocabulary; a stale one — a cadre
	// since retired — is dropped rather than matched against nothing.
	if slug := q.Get("cadre"); slug != "" {
		for _, c := range cadres {
			if c.Slug == slug {
				f.Cadre = slug
				view.Cadre = slug
				break
			}
		}
	}
	switch status := domain.WorkerStatus(q.Get("status")); status {
	case domain.WorkerActive, domain.WorkerInactive:
		f.Status = status
		view.Status = string(status)
	}

	// The location cascade contributes one field per level; the deepest one
	// filled in is the filter, exactly as on the worker form.
	for _, field := range []string{"village_id", "parish_id", "subcounty_id", "district_id"} {
		if id, err := strconv.ParseInt(q.Get(field), 10, 64); err == nil && id > 0 {
			f.LocationID = id
			view.LocationID = id
			break
		}
	}
	view.Active = f.Query != "" || f.Cadre != "" || f.Status != "" || f.LocationID != 0

	f.After = decodeCursor(q.Get("after"))
	f.Before = decodeCursor(q.Get("before"))

	return f, view
}

// cursorSep separates a cursor's three parts. A unit separator, because a
// surname can contain anything a keyboard produces except a control character.
const cursorSep = "\x1f"

// Cursors travel in the URL. They are encoded rather than sent as three
// parameters: it keeps the query string readable, and a position that looks
// opaque is less inviting to hand-edit into nonsense.
func encodeCursor(c *store.Cursor) string {
	if c == nil {
		return ""
	}
	raw := c.LastName + cursorSep + c.FirstName + cursorSep + strconv.FormatInt(c.ID, 10)
	return base64.RawURLEncoding.EncodeToString([]byte(raw))
}

// decodeCursor reads a cursor back, and returns nil for anything malformed —
// a bad cursor means "start at the beginning", not an error page.
//
// A cursor is a position, not a permission: it names a sort key, and the scope
// filter still decides which rows past it are visible.
func decodeCursor(encoded string) *store.Cursor {
	if encoded == "" {
		return nil
	}
	raw, err := base64.RawURLEncoding.DecodeString(encoded)
	if err != nil {
		return nil
	}
	parts := strings.Split(string(raw), cursorSep)
	if len(parts) != 3 {
		return nil
	}
	id, err := strconv.ParseInt(parts[2], 10, 64)
	if err != nil || id <= 0 {
		return nil
	}
	return &store.Cursor{LastName: parts[0], FirstName: parts[1], ID: id}
}

// pageURL rebuilds the current query string around a new cursor, so paging
// keeps the filters and changing a filter starts the paging over.
func pageURL(r *http.Request, exists bool, param string, cursor *store.Cursor) string {
	if !exists || cursor == nil {
		return ""
	}
	q := r.URL.Query()
	q.Del("after")
	q.Del("before")
	q.Set(param, encodeCursor(cursor))
	return "/health-workers?" + q.Encode()
}

// exportURL is the current listing as a file: the same filters, without the
// cursor, because an export is the whole selection rather than the page being
// looked at.
func exportURL(r *http.Request) string {
	q := r.URL.Query()
	q.Del("after")
	q.Del("before")
	if encoded := q.Encode(); encoded != "" {
		return "/health-workers/export.csv?" + encoded
	}
	return "/health-workers/export.csv"
}

func (s *Server) workerShow(w http.ResponseWriter, r *http.Request) {
	id, ok := pathID(r, "id")
	if !ok {
		s.notFound(w, r)
		return
	}
	sc := auth.ScopeFrom(r.Context())

	worker, err := s.store.Workers.Get(r.Context(), sc, id)
	if err != nil {
		s.notFoundOrFail(w, r, err)
		return
	}
	var placement []domain.Place
	if worker.Deployment != nil {
		placement, err = s.store.Locations.Ancestors(r.Context(), sc, worker.Deployment.LocationID)
		if err != nil {
			s.fail(w, r, err)
			return
		}
	}
	profile, err := s.store.Profiles.Get(r.Context(), sc, id)
	if err != nil {
		s.fail(w, r, err)
		return
	}
	tools, err := s.store.Profiles.Tools(r.Context(), id)
	if err != nil {
		s.fail(w, r, err)
		return
	}
	domains, err := s.store.Profiles.ServiceDomains(r.Context(), id)
	if err != nil {
		s.fail(w, r, err)
		return
	}
	history, err := s.store.Deployments.ForWorker(r.Context(), sc, id)
	if err != nil {
		s.fail(w, r, err)
		return
	}

	page := workerShowPage{
		Worker:    worker,
		Placement: placement,
		Profile:   profile,
		Tools:     tools,
		Domains:   domains,
		History:   history,
		CanEdit:   auth.Can(auth.MustUser(r.Context()).Role, auth.CapWorkerUpdate),
	}

	// The supervising facility is an attachment on the open posting. The
	// picker offers the posting's own district, government facilities first.
	if page.CanEdit && worker.Deployment != nil && worker.Deployment.Active() {
		facilities, err := s.store.Deployments.FacilitiesIn(r.Context(), sc, worker.Deployment.DistrictID)
		if err != nil {
			s.fail(w, r, err)
			return
		}
		for _, f := range facilities {
			page.Facilities = append(page.Facilities, facilityOption{
				ID: f.ID, Label: facilityLabel(f.Name, f.Level, f.Ownership),
				Selected: worker.Deployment.FacilityID != nil && *worker.Deployment.FacilityID == f.ID,
			})
		}
	}

	s.render(w, r, http.StatusOK, "worker_show", page)
}

// facilityLabel is one option of the facility picker: the name, its level, and
// the ownership when it is not a government facility — community health workers
// report to government ones, and the rest are marked rather than hidden.
func facilityLabel(name, level, ownership string) string {
	label := name
	if level != "" {
		label += " · " + level
	}
	if ownership != "" && ownership != "GOV" {
		label += " (" + ownership + ")"
	}
	return label
}

func (s *Server) workerNew(w http.ResponseWriter, r *http.Request) {
	blank := domain.HealthWorker{Sex: domain.SexFemale}
	p, err := s.workerForm(r, blank, "/health-workers/new", nil, "")
	if err != nil {
		s.fail(w, r, err)
		return
	}
	s.render(w, r, http.StatusOK, "worker_form", p)
}

func (s *Server) workerCreate(w http.ResponseWriter, r *http.Request) {
	actor := auth.MustUser(r.Context())
	sc := auth.ScopeFrom(r.Context())

	in, age, v := s.decodeWorker(r, sc)
	if v.Any() {
		s.rerenderWorkerForm(w, r, draftWorker(in), "/health-workers/new", v.Fields, age)
		return
	}

	// A name already on the register at the same location is a warning, not a
	// refusal — two people in one village genuinely can share a name — so the
	// admin is shown the matches and confirms.
	if trimmed(r, "confirm_duplicate") == "" {
		dups, err := s.store.Workers.PossibleDuplicates(r.Context(), sc, in.LocationID, in.FirstName, in.LastName, 0)
		if err != nil {
			s.fail(w, r, err)
			return
		}
		if len(dups) > 0 {
			p, err := s.workerForm(r, draftWorker(in), "/health-workers/new", nil, age)
			if err != nil {
				s.fail(w, r, err)
				return
			}
			p.Duplicates = dups
			s.render(w, r, http.StatusOK, "worker_form", p)
			return
		}
	}

	worker, err := s.store.Workers.Create(r.Context(), sc, actor, in, s.clientIP(r))
	if err != nil {
		s.workerWriteFailed(w, r, err, draftWorker(in), "/health-workers/new", age)
		return
	}

	setFlash(w, s.secure(), "ok", worker.FullName()+" has been added to the register.")
	http.Redirect(w, r, workerPath(worker.ID), http.StatusSeeOther)
}

func (s *Server) workerEdit(w http.ResponseWriter, r *http.Request) {
	id, ok := pathID(r, "id")
	if !ok {
		s.notFound(w, r)
		return
	}
	worker, err := s.store.Workers.Get(r.Context(), auth.ScopeFrom(r.Context()), id)
	if err != nil {
		s.notFoundOrFail(w, r, err)
		return
	}

	age := ""
	if worker.AgeYears != nil {
		age = strconv.Itoa(int(*worker.AgeYears))
	}
	p, err := s.workerForm(r, worker, workerPath(id), nil, age)
	if err != nil {
		s.fail(w, r, err)
		return
	}
	s.render(w, r, http.StatusOK, "worker_form", p)
}

func (s *Server) workerUpdate(w http.ResponseWriter, r *http.Request) {
	id, ok := pathID(r, "id")
	if !ok {
		s.notFound(w, r)
		return
	}
	actor := auth.MustUser(r.Context())
	sc := auth.ScopeFrom(r.Context())

	before, err := s.store.Workers.Get(r.Context(), sc, id)
	if err != nil {
		s.notFoundOrFail(w, r, err)
		return
	}

	in, age, v := s.decodeWorker(r, sc)
	// An inactive worker's details can still be corrected, but they take no new
	// posting until reactivated. deployments_set_placement refuses it either
	// way — as a 500 rather than a message.
	if before.Status == domain.WorkerInactive && before.Deployment != nil &&
		(before.Deployment.CadreID != in.CadreID || before.Deployment.LocationID != in.LocationID) {
		v.Add("location", "This worker is inactive. Reactivate them before changing where or as what they serve.")
	}
	if v.Any() {
		draft := draftWorker(in)
		draft.ID = id
		s.rerenderWorkerForm(w, r, draft, workerPath(id), v.Fields, age)
		return
	}

	worker, err := s.store.Workers.Update(r.Context(), sc, actor, id, in, s.clientIP(r))
	if err != nil {
		draft := draftWorker(in)
		draft.ID = id
		s.workerWriteFailed(w, r, err, draft, workerPath(id), age)
		return
	}

	// A cross-district transfer leaves the supervising facility behind: the old
	// posting keeps it in the history, the new one starts unattached.
	note := ""
	if before.Deployment != nil && before.Deployment.FacilityID != nil &&
		(worker.Deployment == nil || worker.Deployment.FacilityID == nil) {
		note = " The supervising facility was not carried into the new district — attach one when ready."
	}

	setFlash(w, s.secure(), "ok", "Saved changes to "+worker.FullName()+"."+note)
	http.Redirect(w, r, workerPath(worker.ID), http.StatusSeeOther)
}

// workerSetFacility attaches, changes or clears the supervising facility on the
// open posting. It is its own act beside the register form: an attachment
// change is not a redeployment, and it audits as deployment.update.
func (s *Server) workerSetFacility(w http.ResponseWriter, r *http.Request) {
	id, ok := pathID(r, "id")
	if !ok {
		s.notFound(w, r)
		return
	}
	actor := auth.MustUser(r.Context())
	sc := auth.ScopeFrom(r.Context())

	worker, err := s.store.Workers.Get(r.Context(), sc, id)
	if err != nil {
		s.notFoundOrFail(w, r, err)
		return
	}
	if worker.Deployment == nil || !worker.Deployment.Active() {
		s.notFound(w, r)
		return
	}

	facilityID, wellFormed := optionalID(r, "facility_id")
	if !wellFormed {
		setFlash(w, s.secure(), "error", "Choose a facility from the list.")
		http.Redirect(w, r, workerPath(id), http.StatusSeeOther)
		return
	}

	// The facility must sit in the posting's own district. The picker only
	// offers that district, so a mismatch means the form was tampered with —
	// deployments_facility_district_trg would refuse it either way, but as a
	// 500 rather than a message.
	if facilityID != nil {
		districtID, err := s.store.Deployments.FacilityDistrict(r.Context(), *facilityID)
		switch {
		case errors.Is(err, domain.ErrNotFound):
			setFlash(w, s.secure(), "error", "That facility no longer exists.")
			http.Redirect(w, r, workerPath(id), http.StatusSeeOther)
			return
		case err != nil:
			s.fail(w, r, err)
			return
		case districtID != worker.Deployment.DistrictID:
			s.forbidden(w, r)
			return
		}
	}

	if err := s.store.Deployments.SetFacility(r.Context(), sc, actor, id, facilityID, s.clientIP(r)); err != nil {
		s.notFoundOrFail(w, r, err)
		return
	}

	setFlash(w, s.secure(), "ok", "Saved the supervising facility for "+worker.FullName()+".")
	http.Redirect(w, r, workerPath(id), http.StatusSeeOther)
}

// workerDeactivate retires a worker from the workforce. The reason is required
// by the handler: the schema only insists that status and timestamp agree, and
// a deactivation without a stated reason is unauditable a year later. The open
// posting ends with the same reason, in the same transaction.
func (s *Server) workerDeactivate(w http.ResponseWriter, r *http.Request) {
	id, ok := pathID(r, "id")
	if !ok {
		s.notFound(w, r)
		return
	}
	actor := auth.MustUser(r.Context())
	sc := auth.ScopeFrom(r.Context())

	reason := trimmed(r, "reason")
	if len(reason) < 5 {
		setFlash(w, s.secure(), "error", "Give a reason for the deactivation — it becomes part of the record.")
		http.Redirect(w, r, workerPath(id), http.StatusSeeOther)
		return
	}

	worker, err := s.store.Workers.Deactivate(r.Context(), sc, actor, id, reason, s.clientIP(r))
	if err != nil {
		s.notFoundOrFail(w, r, err)
		return
	}

	setFlash(w, s.secure(), "ok", worker.FullName()+" is now inactive. The record stays on the register.")
	http.Redirect(w, r, workerPath(id), http.StatusSeeOther)
}

func (s *Server) workerReactivate(w http.ResponseWriter, r *http.Request) {
	id, ok := pathID(r, "id")
	if !ok {
		s.notFound(w, r)
		return
	}
	worker, err := s.store.Workers.Reactivate(r.Context(), auth.ScopeFrom(r.Context()),
		auth.MustUser(r.Context()), id, s.clientIP(r))
	if err != nil {
		s.notFoundOrFail(w, r, err)
		return
	}

	setFlash(w, s.secure(), "ok", worker.FullName()+" is active again. Edit the record to place them.")
	http.Redirect(w, r, workerPath(id), http.StatusSeeOther)
}

// decodeWorker reads and validates the register form: the person and the
// posting. The placement rule is checked here against the location's real
// level and the cadre's own placement_level, so a mismatch comes back as a
// field message; deployments_set_placement is still what guarantees it.
func (s *Server) decodeWorker(r *http.Request, sc auth.Scope) (store.WorkerInput, string, *domain.ValidationError) {
	v := domain.NewValidationError()

	in := store.WorkerInput{
		NIN:       strings.ToUpper(trimmed(r, "nin")),
		FirstName: trimmed(r, "first_name"),
		LastName:  trimmed(r, "last_name"),
		Sex:       domain.Sex(trimmed(r, "sex")),
	}

	if in.FirstName == "" {
		v.Add("first_name", "Enter the first name.")
	}
	if in.LastName == "" {
		v.Add("last_name", "Enter the last name.")
	}
	if !in.Sex.Valid() {
		v.Add("sex", "Choose one.")
	}
	if in.NIN != "" && !domain.ValidNIN(in.NIN) {
		v.Add("nin", "A NIN is 14 characters: two letters, eleven letters or digits, then a letter.")
	}

	// The cadre arrives as its id: cadres are rows now, and the posting's whole
	// placement rule rides on the one chosen.
	var cadre *domain.Cadre
	cadreID, err := strconv.ParseInt(trimmed(r, "cadre_id"), 10, 16)
	if err != nil {
		v.Add("cadre", "Choose a cadre.")
	} else {
		in.CadreID = int16(cadreID)
		cadres, err := s.store.Deployments.Cadres(r.Context())
		switch {
		case err != nil:
			v.Add("cadre", "The cadre list could not be read. Try again.")
		default:
			for _, c := range cadres {
				if c.ID == in.CadreID {
					found := c
					cadre = &found
				}
			}
			if cadre == nil {
				v.Add("cadre", "Choose a cadre from the list.")
			}
		}
	}

	age := trimmed(r, "age_years")
	if age != "" {
		n, err := strconv.Atoi(age)
		if err != nil || !domain.ValidAge(n) {
			v.Add("age_years", "Age must be a whole number between 18 and 99, or left blank.")
		} else {
			years := int16(n)
			in.AgeYears = &years
		}
	}

	// The cascade posts one field per level; the deepest one filled in is the
	// placement. Which one that must be is decided by the cadre's own row.
	in.LocationID = deepestLocation(r)
	switch {
	case in.LocationID == 0 && cadre != nil:
		v.Add("location", "Choose the placement, down to the "+string(cadre.PlacementLevel)+".")
	case in.LocationID == 0:
		v.Add("location", "Choose the placement.")
	case cadre != nil:
		level, districtID, err := s.store.Locations.LevelOf(r.Context(), in.LocationID)
		switch {
		case errors.Is(err, domain.ErrNotFound):
			v.Add("location", "That location no longer exists.")
		case err != nil:
			v.Add("location", "The placement could not be checked. Try again.")
		case level != cadre.PlacementLevel:
			v.Add("location", "A "+cadre.Label+" is placed at "+string(cadre.PlacementLevel)+" level.")
		case !sc.Allows(districtID):
			// Same answer as a location that does not exist: a district user
			// must not map the country by probing ids.
			v.Add("location", "That location no longer exists.")
		}
	}

	return in, age, v
}

// deepestLocation returns the most specific level the cascade posted. Village
// wins over parish, parish over subcounty: the browser leaves the deeper
// selects empty until they are reachable.
func deepestLocation(r *http.Request) int64 {
	for _, field := range []string{"village_id", "parish_id", "subcounty_id", "district_id"} {
		if id, ok := optionalID(r, field); ok && id != nil {
			return *id
		}
	}
	return 0
}

// workerForm assembles the selects and the prefilled placement chain.
func (s *Server) workerForm(r *http.Request, worker domain.HealthWorker, action string, errs map[string]string, age string) (workerFormPage, error) {
	sc := auth.ScopeFrom(r.Context())

	districts, err := s.store.Locations.Districts(r.Context(), sc)
	if err != nil {
		return workerFormPage{}, err
	}

	prefill := map[string]int64{}
	if worker.Deployment != nil && worker.Deployment.LocationID != 0 {
		chain, err := s.store.Locations.Ancestors(r.Context(), sc, worker.Deployment.LocationID)
		if err != nil && !errors.Is(err, domain.ErrNotFound) {
			return workerFormPage{}, err
		}
		for _, place := range chain {
			prefill[string(place.Level)] = place.ID
		}
	}

	selectedSlug := ""
	if worker.Deployment != nil {
		selectedSlug = worker.Deployment.Cadre.Slug
	}
	vocab, err := s.store.Deployments.Cadres(r.Context())
	if err != nil {
		return workerFormPage{}, err
	}
	cadres := cadreOptions(vocab, selectedSlug)
	// A rejected form re-renders from the input, which carries the id rather
	// than the slug — mark by id when there is one.
	if worker.Deployment != nil && worker.Deployment.CadreID != 0 {
		for i := range cadres {
			cadres[i].Selected = cadres[i].ID == worker.Deployment.CadreID
		}
	}
	sexes := make([]sexOption, 0, len(domain.Sexes))
	for _, sex := range domain.Sexes {
		sexes = append(sexes, sexOption{Value: sex, Label: sex.Label(), Selected: sex == worker.Sex})
	}
	opts := make([]districtOption, 0, len(districts))
	for _, d := range districts {
		opts = append(opts, districtOption{
			ID: d.ID, Name: d.Name,
			Selected: prefill[string(domain.LevelDistrict)] == d.ID,
		})
	}
	// A district user has exactly one district; preselect it so the cascade
	// starts one step further along.
	if len(opts) == 1 && !sc.IsNational() {
		opts[0].Selected = true
	}
	if errs == nil {
		errs = map[string]string{}
	}

	return workerFormPage{
		Action:    action,
		Worker:    worker,
		Age:       age,
		Cadres:    cadres,
		Sexes:     sexes,
		Districts: opts,
		Prefill:   prefill,
		Errors:    errs,
	}, nil
}

func (s *Server) rerenderWorkerForm(w http.ResponseWriter, r *http.Request, worker domain.HealthWorker, action string, errs map[string]string, age string) {
	p, err := s.workerForm(r, worker, action, errs, age)
	if err != nil {
		s.fail(w, r, err)
		return
	}
	s.render(w, r, http.StatusUnprocessableEntity, "worker_form", p)
}

// workerWriteFailed turns the write errors a form can provoke into field
// messages, and everything else into a 500.
func (s *Server) workerWriteFailed(w http.ResponseWriter, r *http.Request, err error, draft domain.HealthWorker, action, age string) {
	switch {
	case errors.Is(err, domain.ErrConflict):
		s.rerenderWorkerForm(w, r, draft, action, map[string]string{
			"nin": "That NIN is already on another record.",
		}, age)
	case errors.Is(err, domain.ErrNotFound):
		s.notFound(w, r)
	case errors.Is(err, domain.ErrForbidden):
		s.forbidden(w, r)
	default:
		s.fail(w, r, err)
	}
}

// draftWorker turns rejected input back into a HealthWorker so the form
// redisplays what was typed instead of clearing it.
func draftWorker(in store.WorkerInput) domain.HealthWorker {
	return domain.HealthWorker{
		NIN:       in.NIN,
		FirstName: in.FirstName,
		LastName:  in.LastName,
		Sex:       in.Sex,
		AgeYears:  in.AgeYears,
		Deployment: &domain.Deployment{
			CadreID:    in.CadreID,
			LocationID: in.LocationID,
		},
		Status: domain.WorkerActive,
	}
}

func workerPath(id int64) string { return "/health-workers/" + strconv.FormatInt(id, 10) }
